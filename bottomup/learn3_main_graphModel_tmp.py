...
def Execute(ct,l):
  ...
  with sim.TPause(ct):
    ...
    l.xs.n0['dtheta1']= SSA([0.05])
    # l.xs.n0['dtheta2']= SSA([0.004])
    l.xs.n0['shake_spd']= SSA([1.1])
...
def Run(ct,*args):
  ...
  domain.SpaceDefs={
    ...
    # 'dtheta1': SP('action',1,min=[0.01],max=[0.05]),  #Pouring skill parameter for all skills
    'dtheta2': SP('action',1,min=[0.002],max=[0.01]),  #Pouring skill parameter for 'std_pour'
    'dtheta1': SP('state',1),  #Pouring skill parameter for all skills
    #'dtheta2': SP('state',1),  #Pouring skill parameter for 'std_pour'
    # 'shake_spd': SP('action',1,min=[0.5],max=[1.1]),  #Pouring skill parameter for 'shake_A'
  ...
  domain.Models={
    ...
    #    'dtheta1','shake_spd','shake_axis2'],
      # ['da_total','lp_flow','flow_var'],None],  #Removed 'p_pour'
    'Fflowc_shakeA10': [  #Flow control with shake_A.
      ['lp_pour','size_srcmouth',"dtheta1","shake_spd",'shake_axis2'],
