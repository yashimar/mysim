...
def GenSMShakeA(ct,l,sim):
  ...
  sm['to_max'].ElseAction.Action= lambda: sim.MoveDTheta2(ct,sm.l,sm.l.dtheta1)
  sm['to_max'].ElseAction.NextState= ORIGIN_STATE

  sm.NewState('shake')
  sm['shake'].EntryAction= lambda: sm.l.ChargeTimer(charge_time)
  sm['shake'].NewAction()
  ...
  sm['shake'].Actions[-1].Action= lambda: ( sm.l.ChargeTimer(charge_time), sm.l.Shake(2) )
...
def FlowCGenSM(ct,l,sim):
  ...
  sm['kickback'].ElseAction.Action= lambda: sim.MoveDTheta2(ct,l,-l.dtheta_max)
