...
def Help():
  return '''Primitive action (shake) for ODE grasping and pouring simulation.
  Usage: Expected to be used from an execution context.'''

def GenSMShake(ct,l,sim):
  ...
  sm['to_max'].NewAction()
  sm['to_max'].Actions[-1]= flowed_out_action
  sm['to_max'].NewAction()
  ...
  sm['shake'].NewAction()
  sm['shake'].Actions[-1]= flowed_out_action
  sm['shake'].NewAction()
...
def FlowCGenSM(ct,l,sim):
  ...
  l.sub_sm= TContainer()
  l.sub_sm.shake= GenSMShake(ct,l,sim)
  ...
  sm['init'].ElseAction.Condition= lambda: True
  sm['init'].ElseAction.NextState= 'shake'

  sm.NewState('shake')
  sm['shake'].EntryAction= lambda: (l.sub_sm.shake.Run(), sim.GetSensor(ct,l))
  sm['shake'].ElseAction.Condition= lambda: True
  #sm['shake'].ElseAction.Action= lambda: l.planlearn_callback(ct,l,sim,'shake_end')
  sm['shake'].ElseAction.NextState= 'kickback'
