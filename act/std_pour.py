...
def GenSMStdPour(ct,l,sim):
  ...
  sm.NewState('pour')
  sm['pour'].EntryAction= lambda: sm.l.ChargeTimer(charge_time)  #Time of patience
  sm['pour'].NewAction()
  ...
  sm['pour'].Actions[-1].Action= lambda: ( sm.l.ChargeTimer(charge_time), sim.MoveDTheta(ct,sm.l,0.0) )
