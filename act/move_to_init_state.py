...
def MoveToRcvSM(ct,l,sim):
  def MoveToPourL0():
    ...
    for tc in FRange1(0.0,1.0,Ndiv):
      p_pour = (1.0-tc)*Vec(p_pour0) + tc*Vec(l.p_pour_trg0)
      theta = (1.0-tc)*theta0 + tc*l.theta_init
      # ct.pub.ode_ppour.publish(p_pour_msg)
      # ct.pub.ode_theta.publish(theta_msg)
      sim.SimMove(ct,l,dt=0.04,p_pour=p_pour,theta=theta)
  ...
  sm.StartState= 'start'
  sm.NewState('start')
  sm['start'].ElseAction.Action = lambda: sim.GetSensor(ct,l)
  ...
  sm['move_upward'].Actions[-1].Action= lambda: (sim.MoveDPPour(ct,l,[0.0,0.0,0.01]), sim.GetSensor(ct,l))
