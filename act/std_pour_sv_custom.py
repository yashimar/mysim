...
def GenSMStdPour(ct,l,sim):
  ...
  sm.NewState('pour')
  sm['pour'].EntryAction= lambda: (sim.GetSensor(ct,sm.l), sm.l.ChargeTimer(charge_time))  #Time of patience
  sm['pour'].NewAction()
  ...
  ###
  # sm['pour'].NewAction()
  # sm['pour'].Actions[-1].Condition = lambda: (sm.l.sensors.src_colliding==True or sm.l.sensors.gripper_colliding==True)
  # sm['pour'].Actions[-1].Actions = lambda: (sim.MoveToTrgPPour(ct,sm.l,sm.l.p_pour_trg0,spd=0.06), sim.GetSensor(ct,sm.l))
...
def FlowCGenSM(ct,l,sim):
  ...
  sm['std_pour'].EntryAction= lambda: (l.sub_sm.std_pour.Run(), sim.GetSensor(ct,l))
