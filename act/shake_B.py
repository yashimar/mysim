...
def Help():
  return '''Primitive action (shake_B) for ODE grasping and pouring simulation.
  Usage: Expected to be used from an execution context.'''

def GenSMShakeB(ct,l,sim):
  ...
  def Shake(count):
    def AddVec(rad):
      return Vec([tmpX*math.cos(rad)-tmpZ*math.sin(rad),0.,tmpX*math.sin(rad)+tmpZ*math.cos(rad)])*shake_range
    spd= sm.l.shake_spd
    shake_range = l.shake_range[0] if type(l.shake_range)==list else l.shake_range
    # shake_range = 0.05
    # spd = 8
    p_pour_init= Vec(copy.deepcopy(sm.l.sensors.p_pour))
    tmpX = math.sin(sm.l.sensors.theta)
    tmpZ = math.cos(sm.l.sensors.theta)
    p = p_pour_direction = []
    resolution = 32
    for i in range(resolution):
      p_pour_direction.append(
        p_pour_init 
        + math.cos(2*math.pi/resolution*i)*AddVec(-math.pi/2) 
        + math.sin(2*math.pi/resolution*i)*Vec([0., shake_range, 0.])
      )
    for n in range(count):
      for direction in p_pour_direction:
        for i in range(100):
          if sim.MoveToTrgPPour(ct,sm.l,direction,spd=spd):
            break
...
def FlowCGenSM(ct,l,sim):
  ...
  l.sub_sm= TContainer()
  l.sub_sm.shake_B= GenSMShakeB(ct,l,sim)
  ...
  sm['init'].ElseAction.Condition= lambda: True
  sm['init'].ElseAction.NextState= 'shake_B'

  sm.NewState('shake_B')
  sm['shake_B'].EntryAction= lambda: l.sub_sm.shake_B.Run()
  sm['shake_B'].ElseAction.Condition= lambda: True
  #sm['shake_B'].ElseAction.Action= lambda: l.planlearn_callback(ct,l,sim,'shake_B_end')
  sm['shake_B'].ElseAction.NextState= 'kickback'
...
def Run(ct,*args):
  ...
  l.shake_spd= params['shake_spd_B']
  l.shake_range = params["shake_range"]
