...
def Execute(ct,l):
  ...
  while True:  #Try-and-error starts from here.
    ...
    if not IsSuccess(l.exec_status):
      break
    trial += 1
    if trial >= l.opt_conf["max_trials"]:
      break
...
def Run(ct,*args):
  ...
  l.logdir= '/tmp/dpl2_shake_A{}/'.format(os.environ["ROS_MASTER_URI"].split(":")[-1])
  opt_conf={
  ...
  domain.Graph={
    ...
    'n2c': TDynNode('n2b','P1',('Fflowc_shakeA10','n3sa')),  # choose only shake_A
