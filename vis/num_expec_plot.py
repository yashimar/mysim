...
def RwdPredict(modeldir):
  ...
  d2 = []
  for ex in ex_list:
    d0.append([ex, FRwd.Predict(x=[0.3, ex], x_var=[0.0**2, 0.03**2], with_var=True).Y.item()])
    d1.append([ex, FRwd.Predict(x=[0.3, ex], x_var=[0.0**2, 0.1**2], with_var=True).Y.item()])
    d2.append([ex, FRwd.Predict(x=[0.3, ex], x_var=[0.0**2, 0.2**2], with_var=True).Y.item()])
  d0 = np.array(d0)
  d1 = np.array(d1)
  d2 = np.array(d2)
...
def WolframAlphaResult():
  ...
  #   [3.0, -0.60046]
  # ])
  #-100*max(0,0.3-x)**2 -1.0*max(0,x-0.3)**2, Std[x]=0.03
  d0 = np.array([
    [0, -9.09],
    [0.1, -4.09],
    [0.2, -1.09],
  ...
  #ex) integral_(-100)^(100) (-100*max(0,0.3-(0.6+x))**2-max(0,(0.6+x)-0.3)**2)*1/(sqrt(2*pi)*0.2)*exp(-x**2/(2*0.2**2)) dx
  d2 = np.array([
    [0, -12.9095],
    [0.1, -7.70165],
    [0.2, -4.16983],
...
def Run(ct, *args):
  ...
  modeldir= '/home/yashima/ros_ws/ay_tools/ay_skill_extra/mysim/logs/'\
            +'reward_model/p1_model/Fflowout'
  ...
  expec_wa0, expec_wa1, expec_wa2 = WolframAlphaResult()
  ...
  plt.plot(expec_pred0[:,0], expec_pred0[:,1], label="Std[x]=0.03 (NN model)", linestyle="dashed")
  plt.scatter(expec_wa1[:,0], expec_wa0[:,1], c="red", label="Std[x]=0.03 (wa result)")
  ...
  plt.scatter(expec_wa2[:,0], expec_wa2[:,1], c="red", label="Std[x]=0.2 (wa result)")
  plt.xlim(0.2,0.4)
  plt.ylim(-1.0,0.1)
