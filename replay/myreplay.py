...
def Execute(ct,l):
  ...
  with sim.TPause(ct):  #Pause during plan/learn
    ...
    l.xs.n0['gh_ratio']= SSA(l.seq_list[0]["gh_ratio"][0])
    l.xs.n0['p_pour_trg']= SSA(Vec([l.seq_list[1]["p_pour_trg"][0][0],
                                    l.seq_list[1]["p_pour_trg"][1][0]]))  #A bit above of Center of ps_rcv
  ...
  while True:  #Try-and-error starts from here.
    ...
    try:
      l.xs.n2a['p_pour_trg'] = SSA(Vec([l.seq_list[2*trial+1]["p_pour_trg"][0][0],
                                        l.seq_list[2*trial+1]["p_pour_trg"][1][0]]))
    except:
      break
    ...
    try:
      if "shake_spd" in l.seq_list[2+trial]: 
        selected_skill="shake_A"
        dtheta1= l.seq_list[2+trial]["dtheta1"][0][0]
        shake_spd= l.seq_list[2+trial]["shake_spd"][0][0]
        shake_axis2= [l.seq_list[2+trial]["shake_axis2"][0][0],
                      l.seq_list[2+trial]["shake_axis2"][1][0]]
      elif "dtheta2" in l.seq_list[2+trial]: 
        selected_skill="std_pour"
        dtheta1= l.seq_list[2*(trial+1)]["dtheta1"][0][0]
        dtheta2= l.seq_list[2*(trial+1)]["dtheta2"][0][0]
    except:
      break
...
def ConfigCallback(ct,l,sim):
  ...
  # l.amount_trg= Rand(0.2,0.4)
  l.amount_trg = l.config_list["a_trg"][0][0]
  # l.amount_trg = 0.3
  CPrint(3,'l.amount_trg=',l.amount_trg)
  #l.spilled_stop= 5
  l.spilled_stop= 10
  ...
  l.config.RcvPos= [(l.config_list["ps_rcv"][0][0]+l.config_list["ps_rcv"][3][0])/2,
                    l.config.RcvPos[1], 
                    l.config.RcvPos[2]]
  ...
  l.config.RcvSize = [abs(l.config_list["ps_rcv"][0][0]-l.config_list["ps_rcv"][3][0]), 
                      abs(l.config_list["ps_rcv"][1][0]-l.config_list["ps_rcv"][7][0]), 
                      abs(l.config_list["ps_rcv"][2][0])]
  ...
  l.config.ContactBounce = l.config_list["material2"][0][0]
  l.config.ContactBounceVel = l.config_list["material2"][1][0]
  if l.config_list["material2"][2][0]==1.5: l.config.ViscosityParam1=1.5e-6
  elif l.config_list["material2"][2][0]==0.25: l.config.ViscosityParam1=2.5e-7
  else: l.config.ViscosityParam1=l.config_list["material2"][2][0]
  l.config.ViscosityMaxDist = l.config_list["material2"][3][0]
  l.config.SrcSize2H = l.config_list["size_srcmouth"][0][0]
...
def Run(ct,*args):
  ...
  name = args[0]
  eps_index = args[1]
  num_episodes = args[2] if len(args)==3 else 1
  save_path = "/home/yashima/ros_ws/ay_tools/ay_skill_extra/mysim/logs/"
  f = open(save_path+name+"_sequence_list.yaml", "rb")
  data = yaml.safe_load(f)
  l.seq_list = data[eps_index]["sequence"]
  l.config_list = data[eps_index]["config"]
  # l.mtr_smsz= l.opt_conf['mtr_smsz']
  # l.config.RcvPos = 
  # l.amount_trg= yaml.safe_load(f)[eps_index]["config"]["a_trg"]
  # l.config.RcvSize=
  ...
  l.logdir= '/home/yashima/ros_ws/ay_tools/ay_skill_extra/mysim/logs/myreplay/'+args[0]+"_"+str(args[1])+"/"
  opt_conf={
