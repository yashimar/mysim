...
def Execute(ct,l, count):
  ...
  with sim.TPause(ct):
    ...
    if l.run_type == PLANNING:
      ...
      l.xs.n0['p_pour_trg']= skill_params['p_pour_trg'][count]
...
def Run(ct,*args):
  ...
  domain.SpaceDefs={
    ...
    'shake_spd': SP('action',1,min=[0.2],max=[0.4]),  #Pouring skill parameter for 'shake_A'
    ...
    'shake_axis2': SP('action',2,min=[0.01,-0.2*math.pi],max=[0.03,0.2*math.pi]),  #Pouring skill parameter for 'shake_A'
  ...
  domain.Models={
    ...
    'Fflowc_shakeA10': [  #Flow control with shake_A.
      ['lp_pour','size_srcmouth','shake_spd','shake_axis2'],
      ['da_pour','da_spill2'],None],  #Removed 'p_pour'
    ...
    "Rdamount" : [['da_pour',"da_spill2"],[REWARD_KEY],RwdModel()], 
