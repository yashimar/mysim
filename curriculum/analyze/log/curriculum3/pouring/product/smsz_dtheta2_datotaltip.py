...
def Run(ct, *args):
    ...
    file_name_pref = "relearn_product_"
    pref_relearned_model = ""
    model_name = "Ftip_amount"
    # model = None
    with open(ROOT_PATH+"curriculum3/pouring/relearn/product"+"/{}_{}.pkl".format(model_name,pref_relearned_model), "rb") as f:
        model = pickle.load(f)
    ...
    xs_value = {
        "gh_abs": [0.25],
        "da_trg": [0.3],
        "size_srcmouth": [0.055*10],
        "material2": NOBOUNCE,
        "dtheta1": [1.4e-2],
        "dtheta2": [0.002*50],
    }
    input_features = ["gh_abs","da_trg","size_srcmouth","material2","dtheta1","dtheta2"]
    X = {"feature": "size_srcmouth", "values": np.linspace(0.02*10,0.09*10,100)}
    Y = {"feature": "dtheta2", "values": np.linspace(0.0*50.,0.025*50.,100)}
    ...
    df = pd.DataFrame({
        "dtheta2": np.array(sh["n0"]["dtheta2"][MEAN])*50,
        ...
        "size_srcmouth": np.array(sh["n0"]["size_srcmouth"][MEAN])*10,
