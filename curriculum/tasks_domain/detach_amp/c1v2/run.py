...
def Run(ct, *args):
    ...
    l.logdir_base = ROOT_PATH + "curriculum5/c1v2"+"/"+suff
    ...
    ###########################    
    ### group_task_id: 0
    l.tasks.append(Task(name="mtr=(ketchup,), smsz=(0.03,0.04), Rdapour_gentle + Rdaspill", group_id=5, group_task_id=0))
    l.tasks[-1].config_callback = lambda: custom_config_callback("static",  "custom", ("ketchup",), (0.03, 0.04))
    l.tasks[-1].reward_callback = lambda: update_model({
        "Rdaspill": [['da_spill2'], [REWARD_KEY], Rmodel("Fdaspill")],
        "Rdapour_gentle": [['da_trg', 'da_pour'], [REWARD_KEY], Rmodel("Fdapour_gentle")],
    })
    l.tasks[-1].n_least_episode = 30
    l.tasks[-1].n_consider = 1
    
    ### group_task_id: 1
    l.tasks.append(Task(name="mtr=(ketchup,), smsz=(0.04,0.08), Rdapour_gentle + Rdaspill", group_id=5, group_task_id=1))
    l.tasks[-1].config_callback = lambda: custom_config_callback("static",  "custom", ("ketchup",), (0.04, 0.08))
    l.tasks[-1].reward_callback = lambda: update_model({
        "Rdaspill": [['da_spill2'], [REWARD_KEY], Rmodel("Fdaspill")],
        "Rdapour_gentle": [['da_trg', 'da_pour'], [REWARD_KEY], Rmodel("Fdapour_gentle")],
        "Rskill": [['skill'], [REWARD_KEY], TLocalQuad(1,lambda y: -100.0 if y[0]!=1 else 0)],
