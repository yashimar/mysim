...
def Run(ct, *args):
    ...
    ############################################################################
    t_index = 1
    suff = "t"+str(t_index)+"/first200"+"/"
    l.logdir = ROOT_PATH + "curriculum4/c2/subtask"+"/"+suff
    ...
    ### Group_id: 8
    ###########################
    l.tasks.append(Task(name="mtr=(nobounce,ketchup), smsz=(0.03,0.08), Rdapour_gentle + Rdaspill", group_id=8))
    l.tasks[-1].reward_callback = lambda: update_model({
        "Rdaspill": [['da_spill2'], [REWARD_KEY], Rmodel("Fdaspill")],
